...
# cipher text, restores the plain text; for example, 65 XOR
...
# common English words, decrypt the message and find the sum
...
cipher = [int(x) for x in data.replace('\n', '').split(',')]

def decryption(key, text):
	ret = list(text)
	for i in range(len(text)):
		ret[i] = key[i % 3] ^ text[i]
	return ret

def key_analysis(text):
	key = [max(text[i::3], key=text[i::3].count) for i in range(3)]
	return [x ^ ord(' ') for x in key]

text = decryption(key_analysis(cipher), cipher)
print sum(text)
